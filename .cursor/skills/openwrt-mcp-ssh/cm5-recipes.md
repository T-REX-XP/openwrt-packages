...
**Preconditions:** ESP32 flashed with matching RDCP release; JST UART connected; USB serial unplugged. **Page sync is `evt screen` only** — do not restore `evt input` or echo `cmd screen` on swipe (skill `mcu-display-cm5`).
