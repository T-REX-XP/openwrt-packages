...
- **Feed packages:** `scripts/discover-feed-packages.sh` lists every `feeds/{packages,luci}/*/Makefile`. The SDK job builds that list minus `scripts/sdk-skip-packages.txt` (`suricata`, `luci-app-suricata` — Rust, Docker firmware builder only) and **fails** if any remaining name has no `.apk`. Do not hardcode `PACKAGES`.
